...
const ResourceGrid: React.FC = () => {
  ...
  const { players } = useAppSelector((state) => state.gameConfiguration)
  ...
  const playerColors = new Set(players.map(({ color }) => color))
  const allColors: Array<'blue' | 'red' | 'orange' | 'white'> = [
    'blue',
    'red',
    'orange',
    'white',
  ...
  // Filter the colors array based on player colors
  const colors = allColors.filter((color) => playerColors.has(color)) as Array<
    'blue' | 'red' | 'orange' | 'white'
  >

  const handleChange = (
    row: string,
    resource: 'brick' | 'grain' | 'lumber' | 'ore' | 'wool',
